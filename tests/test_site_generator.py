...
class TestURLFor:
    ...
    def test_mps_list_endpoint(self):
        """Test MPs listing URL generation."""
        url = url_for('mps_list')
        assert url == '/mps/index.html'
    
    def test_mp_profile_endpoint(self):
        """Test MP profile URL generation."""
        url = url_for('mp_profile', mp_id=123)
        assert url == '/mp/123/index.html'
    
    def test_parties_endpoint(self):
        """Test parties listing URL generation."""
        url = url_for('parties')
        assert url == '/parties/index.html'
    
    def test_party_detail_endpoint(self):
        """Test party detail URL generation."""
        url = url_for('party_detail', party_slug='uda')
        assert url == '/party/uda/index.html'
...
class TestSiteGeneration:
    ...
    def test_generate_site_creates_output_dir(self, test_db, temp_output_dir):
        ...
        generate_static_site(db_path=test_db, output_dir=temp_output_dir)
    ...
    def test_generate_site_with_nonexistent_db(self, temp_output_dir):
        ...
        with pytest.raises(SystemExit):
            generate_static_site(db_path='/nonexistent/db.db', output_dir=temp_output_dir)
