...
@given(text1=st.text(min_size=1, max_size=100), text2=st.text(min_size=1, max_size=100))
...
def test_similarity_range_property(generator, text1, text2, text3):
    """
    Property: Similarity must be in range [-epsilon, 1+epsilon] due to floating point precision.
    ...
    **Validates: Requirements 2.4**
    ...
    # Allow small floating point error in both directions
    # Increased epsilon to 0.02 to account for edge cases with very short/unusual Unicode strings
    epsilon = 0.02
    assert -epsilon <= sim12 <= 1.0 + epsilon, f"sim12={sim12} out of range"
    assert -epsilon <= sim23 <= 1.0 + epsilon, f"sim23={sim23} out of range"
    assert -epsilon <= sim13 <= 1.0 + epsilon, f"sim13={sim13} out of range"
